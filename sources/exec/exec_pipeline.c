...
/*   Updated: 2026/06/05 13:36:19 by osousa-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	child_exec(t_shell *shell, t_cmd *cmd, int pipe_in, int pipe_out)
{
...
static pid_t	fork_cmd(t_shell *shell, t_cmd *cmd, int pipe_in, int pipe_out)
{
	...
	if (pid == 0)
		child_exec(shell, cmd, pipe_in, pipe_out);
	if (pipe_in != -1)
		close(pipe_in);
	if (pipe_out != -1)
		close(pipe_out);
...
static int	handle_pipeline_node(t_shell *shell, t_cmd *cmd, int *pipe_in, pid_t *pid)
{
	int	fd[2];
	int	pipe_out;
...
int	exec_pipeline(t_shell *shell, t_cmd *cmd)
{
	...
	{
		if (handle_pipeline_node(shell, cmd, &pipe_in, &pids[n]))
			return (1);
		cmd = cmd->next;
		n++;
	}
